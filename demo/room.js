...
function loadRoom(roomId) {
  ...
  // reset room timer
  if (roomId === 'bedroom') GameState.flags.bedroom_timer = 0;
  if (roomId === 'bathroom') GameState.flags.bathroom_soapTimer = 0;
  // NOTE: Kitchen timers deliberately DO NOT reset here to preserve state across rooms
  if (roomId === 'dining_room') GameState.flags.dining_room_clockTimer = 0;
  if (roomId === 'storage') {
      if(!GameState.flags['storage_doorTimerStarted'] && !GameState.flags['storage_gotHammer']){
          GameState.flags.storage_doorTimer = 0;
          GameState.flags['storage_doorTimerStarted'] = true;
      }
      GameState.flags.storage_panicTimer = 0;
  }
